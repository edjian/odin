 ...
 */
@Service
public class DiditechRedisTokenStore implements TokenStore {
	...
	public DiditechRedisTokenStore(RedisConnectionFactory connectionFactory) {
	...
	private byte[] serializeKey(String object) {
		return serialize(TenantContextHolder.getTenantId() + prefix + object);
	}
	...
	public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
		...
		try {
			...
			// KEY
			byte[] key = serializeKey(String.format("%s%s", SecurityConstants.TOKEN_PREFIX, authentication.getName()));
			// value
		...
		try {
			...
			if (authentication != null) {
				...
				byte[] keyName = serializeKey(
						String.format("%s%s", SecurityConstants.TOKEN_PREFIX, authentication.getName()));
				// 获取redis连接
				conn.del(keyName);
				conn.closePipeline();
			}
