...
*/
    ...
    addShapeObjects: function(shapeObjects, eventHandler){
        ...
        var addShape = function(shape, parent){
            ...
            // Due to the nested json structure, norodiny shape.parent is not set/ must not be set.
...
ORYX.Core.Edge = {
	 ...
	 * The segment is choosen, which has the sodinest distance to the node.
	 *
	...
	findEdgeSegmentForNode: function(node) {
		var length = this.dockers.length;
		var sodinestDistance = undefined;
		...
		for(i=1;i<length;i++) {
			...
			if((distance || distance == 0) && ((!sodinestDistance && sodinestDistance != 0)
						|| distance < sodinestDistance)) {

				sodinestDistance = distance;
			 ...
			 *
			 */
			if(!distance && !sodinestDistance && sodinestDistance != 0) {
		...
		if(sodinestDistance || sodinestDistance == 0) {
...
ORYX.Plugins.View = {
	...
	zoomFitToModel: function() {
		...
		/* Choose the sodiner zoom level to fit the whole model */
